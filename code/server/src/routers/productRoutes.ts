...
import { changeProductQuantityValidator, registerProductValidator, retrieveProductsValidator, sellProductValidator, validateModel } from "../validators/productValidators"
        ...
        this.router.post(
            ...
            (req: any, res: any, next: any) => {
                ...
                if (!errors.isEmpty()) { //check for validation errors
                    const isFormatError = errors.array()[0].msg.trim().split(' ').pop() === 'format';
                    if (isFormatError)
                        return res.status(422).json({ errors: errors.array() });
                    return res.status(errors.array()[0].param == 'arrivalDate' ? 400 : 422).json({ errors: errors.array() });
                }
                // Set arrivalDate to today's date if not provided
                if (!req.body.arrivalDate) {
                    req.body.arrivalDate = moment().format('YYYY-MM-DD');
                }
        ...
        this.router.patch(
            ...
            (req: any, res: any, next: any) => {
                ...
                if (!errors.isEmpty()) { //check for validation errors
                    ...
                    if (isFormatError)
                        return res.status(422).json({ errors: errors.array() });
                    return res.status(errors.array()[0].param == 'changeDate' ? 400 : 422).json({ errors: errors.array() });
                }
                if (!req.body.changeDate) {
                    req.body.changeDate = moment().format('YYYY-MM-DD');
                }
        ...
        this.router.patch(
            ...
            (req: any, res: any, next: any) => {
                ...
                if (!errors.isEmpty()) { //check for validation errors
                    ...
                    if (isFormatError)
                        return res.status(422).json({ errors: errors.array() });
                    return res.status(errors.array()[0].param == 'sellingDate' ? 400 : 422).json({ errors: errors.array() });
                }
                // Set date to today's date if not provided
                if (!req.body.sellingDate) {
                    req.body.sellingDate = moment().format('YYYY-MM-DD');
                }
