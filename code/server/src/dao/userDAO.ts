...
import { BadRequestError, UserAlreadyExistsError, UserNotFoundError } from "../errors/userError";
import { rejects } from "assert";
import { error } from "console";
import { resolve } from "path";


function mapRowsToFilms(rows: any[]): User[] {
    return rows.map(row => new User(row.username, row.name, row.surname, row.role, row.address, row.birthdate));
}
...
class UserDAO {
    ...
    deleteAllUsers(roleToEvit: String): Promise<Boolean> {
        return new Promise<Boolean>((resolve, reject) => {
            try {
                const sql2 = "DELETE FROM users WHERE role <> ?"
                db.run(sql2, [roleToEvit], (err: Error | null) => {
                    if (err) {
                        reject(err)
                        return
                    }
